...
//	CONTAINER(char *nombre);
	...
	~CONTAINER(){};
	//Lectura de datos
	void LeerDatosContainer(char *nombre, int opcion);
	//Escritura de datos
	...
	void Grasp_Ceschia();
	Calculator ConfiguracaoToCalculator(list<CONFIGURACAO> configu);
	vector<vector<int>> ConfiguracaoToVector(CONFIGURACAO bloqueCajas);
...
public:
	...
	//destructor
	~PIECE(){};
	bool PIECE::operator < ( const PIECE& p2);  // Estaba comentada en el codigo inicial 
